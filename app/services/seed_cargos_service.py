"""Carga masiva de cargos desde lista o archivo (sin normalizar texto)."""
...
DEFAULT_CARGOS_SEED_PATH = Path(__file__).resolve().parent.parent / "data" / "cargos_seed.txt"
CHUNK_SIZE = 500


def read_cargos_lines_from_file(path: Path) -> list[str]:
    """Lee líneas del archivo; solo quita el salto de línea final de cada línea (no strip del contenido)."""
    raw = path.read_text(encoding="utf-8")
    return [line.removesuffix("\r\n").removesuffix("\n") for line in raw.splitlines()]


def seed_cargos_bulk_insert(session: Session, nombres: list[str]) -> dict[str, int]:
    """
    ...
    for i in range(0, len(rows), CHUNK_SIZE):
        ...
        stmt = insert(Cargo).values(chunk)
